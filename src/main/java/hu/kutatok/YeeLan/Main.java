...
import static hu.kutatok.YeeLan.Command.COMMANDS;
import static hu.kutatok.YeeLan.ParameterType.ENUM;
import static hu.kutatok.YeeLan.ParameterType.NULL;
...
public class Main
{
    ...
    public static void main(String... args)
    {
        ...
        String[] commandArray = COMMANDS.keySet().toArray(new String[0]);
    ...
    private static String composeParams(String command)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                paramsJoiner
                        .add(requestParameter(parameters[i]));
            }
        }
    ...
    private static String requestParameter(Parameter parameter)
    {
        switch (parameter.type)
        {
            ...
            case INT_ENUM:
            case INDEXED_ENUM:
                System.out.printf(ENUM_REQUEST_TEMPLATE,
                        parameter.getValuesAsString(),
                        parameter.name
                );
                break;
                
            default:
                System.err.println("Not suitable parameter! @ hu.kutatok.YeeLan.Main.requestParameter()");
                return "";
        }
